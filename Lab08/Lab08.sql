...
INSERT INTO Game VALUES (5, '2018-11-30 13:21:00');
...
SELECT * 
 	FROM Game
	Where time >= '2018-11-29 13:21:00'   
;
...
SELECT name,score
	FROM PlayerGame
	INNER JOIN Player on Player.ID = PlayerGame.playerID
	WHERE playerID = 2
	ORDER BY score DESC
;
...
SELECT name,score
	FROM PlayerGame
	INNER JOIN Player on Player.ID = PlayerGame.gameID
	WHERE gameID = 2
	ORDER BY score DESC
	LIMIT 1
;
...
--c. The Clause limits the output name to ID numbers less than P1's
--d. When a table may have various instances of an object (name, address, etc...) 
--but unqiue Keys, we can use that kind of join to retreive the solo desired results 
*/